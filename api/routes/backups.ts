...
router.get('/:sectionKey', async (req: AuthRequest, res: Response) => {
  try {
...
router.post('/', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const version = (versionResult.rows[0] as any).next_version
...
router.post('/restore', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const value = JSON.parse(backup.value as string)
